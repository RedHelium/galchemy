...
pub fn to_pog_value(value: expression.SqlValue) -> pog.Value {
  case value {
    ...
    expression.Bytea(v) -> pog.bytea(v)
    expression.Uuid(v) -> pog.text(v)
    expression.Numeric(v) -> pog.text(v)
    expression.Json(v) -> pog.text(v)
    expression.Jsonb(v) -> pog.text(v)
    expression.Enum(type_name: _, value: v) -> pog.text(v)
    expression.Array(v) -> pog.array(to_pog_value, v)
